...
func (RoleDocApi) RoleDocTreeView(c *gin.Context) {
	...
	// 设置缓存
	redis_service.SetRoleDocTree(roleID, userID, response.List)
	res.OKWithData(response, c)
}

// RoleDocTreeTransition 角色文档树 转换为特定类型
func RoleDocTreeTransition(docList []*models.DocModel, docPwdMap, docSeeMap, docCollMap, unLuckMap map[uint]bool) (list []redis_service.RoleDocTree) {
	for _, model := range docList {
